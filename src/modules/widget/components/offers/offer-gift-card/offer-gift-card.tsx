...
import { selectChosenOffer } from '../../../../../slices/checkout-slice';
...
export const OfferGiftCard: React.FC<OfferGiftCardProps> = ({ offer, onClickHandler }): React.ReactElement => {
    let activeOfferId;
    const currentChosenOffer = useAppSelector(selectChosenOffer);

    if (currentChosenOffer) {
        activeOfferId = currentChosenOffer?.giftcard_list[0]?.checkout_value_id;
    }
